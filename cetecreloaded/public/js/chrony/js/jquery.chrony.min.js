...
;(function($){
			...
			init:function(options){
				return this.each(function(){
					...
					if(opt.text){
						...
						opt.second=text[2];opt.minute=text[1];
						opt.hour=text[0];
					}else{
						if(opt.hours){
							if(opt.hours>=24){
					...
					timer=setInterval(function(){
						...
						if(second==0){
							if(minute==0){
								if(hour>0){
									...
									minute=59;second=59;$hour.html(hour);
									$minute.html(minute);$second.html(second);
								}else{
									//califExam();
								}
							}
						...
						if(self.opt.finish&&second==0&&minute==0&&hour==0){
							self.opt.finish.call(self);clearInterval(timer);
						}
						methods.checkAlert.call(self,hour,minute,second);
					},1000);
				});
	...
	$.fn.chrony.defaults={
			alert:{
				color:"#F00",hour:0,minute:0,second:10
			},blink:false,blinkTime:130,finish:undefined,decrement:1,displayHours:true,displayMinutes:true,displaySeconds:true,hour:0,hours:undefined,minute:0,minutes:undefined,second:0,seconds:undefined,text:undefined
		};
	})(jQuery);
