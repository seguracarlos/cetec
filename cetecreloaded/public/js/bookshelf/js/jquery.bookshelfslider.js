...
(function($){
	...
	$.bookshelfSlider = function(selector, settings,defaultPosition) {
		...
		//var arrow_menu_pos = $(selector + " #btn99").position().left + marginLeft + ($(selector + " #btn99").width() / 2) - 10;
		var arrow_menu_pos = $(selector + defaultPosition).position().left + marginLeft + ($(selector + defaultPosition).width() / 2) - 10;
		...
		if ($bsc.show_selected_title)	{ 
			...
			$(selector + ' .selected_title').text( $(defaultPosition).text() );
		}
		...
		$(selector + defaultPosition).trigger('click');
